...
const AppSettings = (props) => {
    ...
    const blockUser = (userNameToBlock) => {
        ...
        axios
            .post(`http://${process.env.REACT_APP_ORIGIN}/blockuser`, {
                addBlock: true,
    ...
    useEffect(() => {
        axios(
            `http://${process.env.REACT_APP_ORIGIN}/users?userID=${currentUser._id}`
        )
            .then((response) => {
                setBlockedUsers(
    ...
    const handleChangedNotifSwitch = () => {
        axios
            .post(
                `http://${process.env.REACT_APP_ORIGIN}/notificationsettings`,
                {
    ...
    const handleAddBlockedTag = (tagToBlock) => {
        if (blockedTagsList.includes(tagToBlock) === false) {
            axios
                .post(`http://${process.env.REACT_APP_ORIGIN}/blocktag`, {
                    addBlock: true,
    ...
    const signOutUser = () => {
        axios
            .post(`http://${process.env.REACT_APP_ORIGIN}/signout`)
            .then(() => {
