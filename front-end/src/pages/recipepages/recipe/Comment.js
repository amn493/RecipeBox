...
const Comment = (props) => {
    ...
    useEffect(() => {
        axios(
            `https://${process.env.REACT_APP_ORIGIN}/userbyid?id=${comment.user}`
        )
            .then((response) => {
    ...
    useEffect(() => {
        if (comment.replyTo) {
            axios(
                `https://${process.env.REACT_APP_ORIGIN}/userbyid?id=${comment.replyTo}`
            )
                .then((response) => {
    ...
    const handleDeleteComment = () => {
        if (window.confirm('Are you sure you want to delete this comment?')) {
            ...
            axios
                .post(
                    `https://${process.env.REACT_APP_ORIGIN}/deletecomment`,
                    {
                        id: comment._id,
