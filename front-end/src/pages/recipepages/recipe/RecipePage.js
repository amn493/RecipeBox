...
const RecipePage = (props) => {
    ...
    useEffect(() => {
        ...
        axios(`http://${process.env.REACT_APP_ORIGIN}/recipe?slug=${slug}`)
            .then((response) => {
    ...
    useEffect(() => {
        if (recipe) {
            if (props.user._id === recipe.user) {
                ...
                axios(
                    `http://${process.env.REACT_APP_ORIGIN}/userbyid?id=${recipe.user}`
                )
                    .then((response) => {
                        setAuthorUser({
    ...
    useEffect(() => {
        if (recipe) {
            axios(
                `http://${process.env.REACT_APP_ORIGIN}/comments?recipeID=${recipe._id}`
            )
                .then((response) => {
    ...
    const handleDelete = () => {
        if (window.confirm('Are you sure you want to delete this recipe?')) {
            ...
            axios
                .post(
                    `http://${process.env.REACT_APP_ORIGIN}/deleterecipe`,
                    {
                        id: recipe._id
                    }
                )
    ...
    const handlePin = () => {
        axios
            .post(`http://${process.env.REACT_APP_ORIGIN}/pinrecipe`, {
                id: recipe._id,
                pin: !recipe.pinned
            })
            .then((response) => {
    ...
    const handleSubmit = (event) => {
        ...
        if (props.signedIn) {
            ...
            if (value !== '') {
                ...
                axios
                    .post(
                        `http://${process.env.REACT_APP_ORIGIN}/comment`,
                        newComment
                    )
                    .then((response) => {
